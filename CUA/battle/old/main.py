...
def findBoss(enemyNotorieties = ['Attackable', 'Enemy', 'Gray', 'Criminal', 'Murderer'], maxDistance = 32):
...
def FindEnemy():
    UnsetAlias('enemy')
    enemy = GetEnemy(['Murderer', 'Enemy', 'criminal', 'gray'], 'Any', 'Closest')
    if not FindObject('enemy', 16):
        UnsetAlias('enemy')
        enemy = None
        Stop()
    return enemy
