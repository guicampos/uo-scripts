...
WAITING = 25  # Set depending on your ping
HEAL_AT = 220  # HP
...
class Enemies:
    ...
    def refresh(self, search_distance=10):
    ...
    def _find_enemies(self, distance=6):
        
        enemies =   Engine.Mobiles.Where(lambda m: m.Distance <= 2
                                              and m.Serial != Engine.Player.Serial
                                              and (['Attackable', 'Enemy', 'Gray', 'Criminal', 'Murderer'] == None 
                                              or ['Attackable', 'Enemy', 'Gray', 'Criminal', 'Murderer'].Contains(m.Notoriety.ToString()))).OrderBy(lambda m: m.Distance)
        return enemies
    ...
    def __init__(self, enemies):
        ...
        #              SpellFn.CHIVALRY, GetAlias('self')),
            ...
            SpellInfo('Enemy Of One', 20, 45, WAITING, 4,
                      ...
                      SpellFn.CHIVALRY),
            SpellInfo('Consecrate Weapon', 10, 15, 1000, 2,
                      self._cast_consecrate_weapon,
                      SpellFn.CHIVALRY),
            SpellInfo('Whirlwind', 30, 90, WAITING, 1,
                      self._cast_whirlwind,
                      SpellFn.SECONDARY_ATTACK),
            SpellInfo('Lightning Strike', 10, 50, WAITING, 1,
