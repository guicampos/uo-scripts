...
def searchArea(): #search filter function
    filter = Mobiles.Filter()#set filter variable
    filter.Enabled = True #is filter on?
    filter.RangeMax = scanrange #range to add to filter
    filter.Notorieties = List[Byte](bytes([3,4,5,6])) #mobile notorieties
    enemies = Mobiles.ApplyFilter(filter) # apply filter to list
